...
# ChatDev/puppeteer/inference/reasoning/reasoning.py -> ChatDev/puppeteer/
...
class GraphReasoning:
    def __init__(self, task:json, graph: AgentGraph, env=None, env_name=None):
        ...
        self.global_logger = LogManager(config_path, self.task.get("type"))
    ...
    def aggregate_answers(self, global_info, answers:list, query_func=None) -> str:
        ...
        elif self.task.get("type") == "OpenDiggerMMLU":
            answer_prompt = "\n".join(prompt["OpenDigger_aggregation"]).format(str(["{}\n".format(answer) for answer in answers]))
        else: 
    ...
    def finalize(self):
        ...
        for idx, reasoning_path in enumerate(self.reasoning_paths):
            ...
            if self.task.get("type") == "MMLU-Pro":
                ...
                main_logger.info(metrics)
                self.policy.finalize_task(transition, reasoning_path.global_info)
            elif self.task.get("type") == "OpenDiggerAnalysis":
                # OpenDigger Reward Logic
                # 1. Check if the output is a valid JSON
                # 2. Check if all required keys are present
                # 3. Check if 'governance_advice' is not empty
                try:
                    import json
                    if isinstance(aggregated_answer, str):
                        # Try to find JSON block
                        if "```json" in aggregated_answer:
                            json_str = aggregated_answer.split("```json")[1].split("```")[0]
                        elif "```" in aggregated_answer:
                            json_str = aggregated_answer.split("```")[1].split("```")[0]
                        else:
                            json_str = aggregated_answer
                        data = json.loads(json_str)
                    else:
                        data = aggregated_answer
                    
                    required_keys = ['analysis_summary', 'prediction_next_month', 'explanation', 'governance_advice']
                    missing_keys = [k for k in required_keys if k not in data]
                    
                    if missing_keys:
                        reward = -0.5  # Penalize for missing keys
                        metrics = {"status": "missing_keys", "missing": missing_keys}
                    else:
                        reward = 1.0   # Success
                        metrics = {"status": "success"}
                        
                        # Bonus: Check if prediction is numeric
                        pred = data.get('prediction_next_month', {})
                        if isinstance(pred.get('next_month_openrank'), (int, float)):
                            reward += 0.2
                        
                except Exception as e:
                    reward = -1.0 # Failed to parse JSON
                    metrics = {"status": "json_parse_error", "error": str(e)}
                ...
                main_logger.info(f"OpenDigger Reward: {reward}, Metrics: {metrics}")
                self.policy.finalize_task(transition, reasoning_path.global_info)
    ...
    def visualize_graph(self):
        ...
        logger.info(f"Graphs saved to: {self.workspace_path}")

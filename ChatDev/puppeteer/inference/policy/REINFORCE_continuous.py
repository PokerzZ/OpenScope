...
# ChatDev/puppeteer/inference/policy/REINFORCE_continuous.py -> ChatDev/puppeteer/
...
class MLP_PolicyNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # Ensure input dimension matches the first layer
        if x.shape[1] != self.input_dim:
             # If input is larger (e.g. 8192) and we expect smaller (e.g. 4096), or vice versa
             # We need to re-initialize the network or project the input.
             # Since this is a Singleton and initialized once, we might need a dynamic adjustment or a projection layer.
             # For simplicity in this fix, we'll assume the input_dim passed to __init__ was correct for the embedding model used.
             # If we are here, it means the embedding model output dim (8192) doesn't match what was expected (4096 maybe?).
             # Let's check where input_dim comes from. It comes from self.state_representation.dim.
             # In embedding.py, RewardModelTokenRepresentation.dim returns 8192.
             # So input_dim should be 8192.
             # The error says: mat1 and mat2 shapes cannot be multiplied (1x4096 and 8192x512)
             # This means x is 1x4096, but self.fc1 expects 8192 input features.
             # Wait, the error is (1x4096 and 8192x512). 
             # mat1 is x (1x4096). mat2 is weights (8192x512).
             # So x has 4096 features, but the model expects 8192.
             # This implies the embedding model returned a 4096-dim vector, but claimed to return 8192.
             # Let's look at embedding.py again.
             pass
...
class ContinuousREINFORCE(LearningPolicy):
    def __init__(self, agent_graph, action_graph, config_path="config/policy.json"):
        ...
        # If using default relative path, convert to absolute path
        if config_path == "config/policy.json":
            # Use the project_root defined at module level or calculate it
            current_dir = os.path.dirname(os.path.abspath(__file__))
            root = os.path.abspath(os.path.join(current_dir, "../.."))
            config_path = os.path.join(root, "config/policy.json")
        ...
        self.dataset_mode = self.config.get("dataset_mode", None)
        ...
        if self.dataset_mode == "train":
            ...
            # Fallback to config['training']['training'] if dataset_mode is not set
            self.training = self.config["training"].get("training", False)
    ...
    def update(self):
        ...
        if len(self.executed_trajectories) >= self.sample_size:
            ...
            for trajectories in self.executed_trajectories[:self.sample_size]:
                ...
                if len(task_avg_metrics) == 0:
                    ...
                    for key in task_avg_metrics[0].keys():
                        # Check if the value is numeric (int, float, or tensor)
                        val = task_avg_metrics[0][key]
                        is_numeric = isinstance(val, (int, float)) or (isinstance(val, torch.Tensor) and val.numel() == 1)
                        ...
                        # Calculate average
                        values = [m[key] for m in task_avg_metrics]
                        if isinstance(values[0], torch.Tensor):
                            avg_val = torch.stack(values).mean()
                        else:
                            avg_val = sum(values) / len(values)
                        episode_metrics[key].append(avg_val)
            ...
            if policy_loss: 
                ...
                metrics.update({f'reasoning/{key}': np.mean([safe_item(r) for r in episode_metrics[key]]) for key in episode_metrics})
                logger.info("metrics: {}".format(metrics))  
                self.global_step += 1
                ...
                self.entropy_history = []
                self.execution_count = 0
                self.previous_task = None  # Reset previous task to ensure new trajectories are created
                self.reward_from_rm = []
                self.action_probs_history = []
                self.llm_action_probs_history = []
                return {
